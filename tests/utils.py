...
def rand_df(shape=(100, 100), labeled=True, y_label='label', outlier=False):
    if outlier:
        data = np.random.uniform(low=-100, high=100, size=(shape[0] - 1, shape[1]))
...
def rand_df_classification(shape=(100, 100), y_label='label', classes=2, outlier=False):
    if outlier:
        X_data = np.random.uniform(low=-100, high=100, size=(shape[0] - 1, shape[1] - 1))
